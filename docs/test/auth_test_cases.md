...
- **前提:**
    ...
    - プロトタイプでは、自身のメールアドレス（プラスアドレッシング等）を使用してメール受信を確認する。
...
- **検証手順:**
    1.  **正常系フロー (Happy Path):**
        ...
        3.  **メール受信 (Manual):**
            - 登録したメールアドレスの受信トレイを確認。
            - 認証メールが届いているか確認。
        4.  **認証完了 (Verify):**
            - メール内のリンクをクリックする（またはURLをブラウザに貼り付け）。
            - `/verified` (認証完了画面) へリダイレクトされることを確認。
            - 画面に「認証が完了しました」と表示されていることを確認。
            - **ログアウト確認**: この時点で自動ログインされて**いない**ことを確認（セッションがないこと）。
        5.  **ログイン (Login):**
            - 「ログイン画面へ」ボタンをクリック。
            - `/login` からメールアドレスとパスワードで正常にログインできることを確認。
            - `/main` へ遷移することを確認。
            - **DB確認**: `email_confirmed_at` に日時が記録されていること。
    2.  **異常系フロー (Error Cases):**
        1.  **重複登録 (Already Registered):**
            - 認証済みユーザーと同じメールアドレスで再度新規登録を行う。
            - 「すでに登録されているメールアドレスです」のエラーが表示されることを確認。
        2.  **無効リンク (Invalid Token):**
            - メール認証リンクのトークン部分（token=...）を適当な文字列に書き換えてアクセスする。
            - 認証が成功せず、エラー画面（または404）が表示されることを確認。
- **合格基準:**
    - 未認証状態でログインできないこと。
    - 認証リンク踏破後に完了画面が表示され、その後手動でログインが可能になること。
    - DBのステータスが適切に遷移すること。
