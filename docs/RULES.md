...
1.  **READ_ONLY_MODE (質問・相談・雑談・調査)**
    -   **トリガー**:
        -   ユーザーが情報または意見を求めている場合
        -   「まだ実行しないで」「コードは書かなくていい」等の制約指示がある場合
        -   明確に「?」「？」などで質問されている場合
    ...
    -   **挙動**: 
        -   「トリガー」に該当するか否か判別し、該当する場合は、テキストのみで回答する。
        -   「トリガー」に該当するか否か判別し、該当する場合は、絶対に「作成」「実装」「修正」作業を行ってはならない。
...
1.  **Requirement Guard (要件・指示の門番):** `.agent/skills/agent_requirement_guard/SKILL.md`
2.  **FactChecker (事実確認担当):** `.agent/skills/agent_fact_checker/SKILL.md`
3.  **Architect (設計・仕様責任者):** `.agent/skills/agent_architect/SKILL.md`
4.  **Test Engineer (テスト設計責任者):** `.agent/skills/agent_test_engineer/SKILL.md`
5.  **Builder (実装責任者):** `.agent/skills/agent_builder/SKILL.md`
6.  **QA Master (品質管理責任者):** `.agent/skills/agent_qa_master/SKILL.md`
...
- **Assignment Verification (アサイン時の定義確認):**
    - 担当エージェントを決定する際は、必ずそのエージェントのスキル定義ファイル（`.agent/skills/*/SKILL.md`）を `read` する。
    - **Protocol Refresh:** ファイルを `read` しても、その内容を数ターンで忘れることを前提に行動せよ。重要な制約（特に禁止事項）は、実行直前の思考ログ（`<analysis>` 等）に「現在の制約: XXXX」として明記・転記してからツールを使用すること。

### 10.3 Execution Protocol (なりきり義務)
タスク実行者は、計画書で指定された担当エージェントタグ（例: `[/agent_test_engineer]`）に到達した際、**必ず** 以下の手順を踏まなければならない。

1.  **Context Switch:** 指定されたスキル定義ファイル（例: `.agent/skills/agent_test_engineer/SKILL.md`）の内容を参照・ロードする。
2.  **Role Play & Declaration (宣誓義務):** 
    - その定義ファイルに書かれた「目的」「制約事項」「Checklist」を厳守する。
    - **義務:** 該当スキルファイルを読み込んだ直後、必ず `<context_switch role="[スキル名]" status="active" />` というタグを含んだ思考ログを出力し、自身のモードが切り替わったことを明示すること。
...
6.  **通知はクリーンか？**: `notify_user` のメッセージに `\n` (エスケープ文字) が混入していないか？（改行はMarkdownで行うこと）

**違反の兆候がある場合は、直ちに行動を修正せよ。**

## 12. System Interaction Guidelines
### 12.1 Notification Formatting
- **Constraint:** `notify_user` の `Message` 引数を作成する際、改行コードのリテラル表現（`\n`）を使用してはならない。
- **Requirement:** 改行が必要な場合は、Markdownの仕様に従い、実際の改行または二重空白を使用すること。
- **Reason:** ユーザーインターフェース上でエスケープ文字がそのまま表示され、可読性を損なうのを防ぐため。